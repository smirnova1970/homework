...
class UrTube:
    ...
    def register(self, nickname: str, password: str, age: int) -> None: #Метод register, который принимает три
    ...
    def  get_videos(self, text: str):     # принимает поисковое слово и возвращает список названий всех видео, содержащих
# поисковое слово не учитывать регистр
    ...
    def watch_video(self, movie: str) -> None:   # Метод watch_video, который принимает название фильма, если
            # не находит точного совпадения(вплоть до пробела), то ничего не воспроизводится, если же находит, ведётся отчёт
            # в консоль на какой секунде ведётся просмотр. После текущее время просмотра данного видео сбрасывается.
